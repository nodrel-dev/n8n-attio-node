...
- Validation result: all items pass. The spec stays at the user-value level — the operation
  matrix is expressed as named user actions (not API paths), and API field names and routing
  mechanics are deferred to `/speckit-plan` per the brief.
- HTTP method semantics (PATCH/PUT) are intentionally referenced only as user-facing
  Append/Overwrite behavior, keeping the spec implementation-agnostic.

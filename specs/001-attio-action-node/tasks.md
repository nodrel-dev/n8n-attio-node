...
- [X] T001 Clear **Decision Gate 0** (research.md R6, brief §3): confirm with the n8n Creator Portal that a fresh hand-built package under the Nodrel identity is accepted for verification; capture the final package name (shipped as `@nodrel-dev/n8n-nodes-attio`). **Blocks all scaffolding.**
- [X] T002 Scaffold the node with `npm create @n8n/node` (depends on T001 for the package name)
- [X] T003 Configure `package.json`: final name (T001), `n8n` block (`n8nNodesApiVersion: 1`, `credentials: ["dist/credentials/AttioApi.credentials.js"]`, `nodes: ["dist/nodes/Attio/Attio.node.js"]`), `keywords` incl. `n8n-community-node-package`, `engines.node >= 22.22`, **`dependencies: {}`**, devDep `@n8n/node-cli >= 0.23.0` (now `^0.46.4`)
...
- [X] T008 Add `.github/workflows/ci.yml`: `npm ci`, `npm run lint`, `npx @n8n/scan-community-package @nodrel-dev/n8n-nodes-attio`, `tsc --noEmit`, `npm run build`, `npm test`, **zero-dep gate** (`node -e "process.exit(Object.keys(require('./package.json').dependencies||{}).length?1:0)"`), **no-env/no-fs gate** (grep `nodes/` for `process.env` and `require('fs')`/`from 'fs'`/`node:fs`; fail if any match, per FR-010/Principle X), PR-title commitlint; top-level `permissions: contents: read`
...
- [X] T010 [P] Add/confirm `.github/workflows/publish.yml` (scaffold provenance publisher; `on: release: types: [published]` — **superseded: that trigger never fires for `GITHUB_TOKEN` releases; `publish.yml` is now `workflow_call`-only, invoked by `release-please.yml`**; `permissions: contents: read, id-token: write`; `npm publish --provenance --access public`)
...
- [X] T015 [P] Write table-driven tests for `formatAttioError` (400/401, 403→likely-missing-scope, 404, 429→rate-limit + `Retry-After` as **date**; never echoes token) in `test/core/formatAttioError.test.ts` — **the date-only premise was wrong; corrected in T088** (both RFC 9110 forms, delta-seconds ruled out first)
...
- [X] T088 **Verify-live**: a 429 surfaces a rate-limit message and `Retry-After` is parsed correctly; README directs users to n8n Retry-On-Fail (research.md R4, NFR-10). **Closed 2026-08-17 — and it found a real bug.** A live 429 was reproduced against the real API (Attio rate-limits **per endpoint**, and the limit for `POST /v2/objects/{object}/records/query` trips on *concurrency* and decays in ~1-2s, so the burst must be in flight as the node fires; `GET /v2/self` is much cheaper to exhaust and holds for minutes). Driven end-to-end through the node in local n8n: the NDV showed `Attio API error 429 (rate_limit_error/rate_limit_exceeded): Rate limit exceeded, please try again later Rate limited by Attio. Retry after 2026-08-17T14:35:54.000Z (Retry-After: Mon, 17 Aug 2026 14:35:54 GMT). Enable n8n "Retry On Fail" …`. **The bug:** this task (and every unit test) assumed `Retry-After` is *always* an HTTP date. Attio actually sends **both** forms per RFC 9110 §10.2.3 — `/v2/self` returned `Mon, 17 Aug 2026 14:30:00 GMT`, the records-query limiter returned **`9`**. Because `new Date('9')` is a *valid* Date, the seconds form rendered live as **`Retry after 2001-09-01T00:00:00.000Z`** (and `0` → `2000-01-01`) — an authoritative-looking timestamp 25 years in the past. `buildRateLimitHint` now detects delta-seconds before attempting a date parse; 5 new unit tests pin both forms plus zero/large/whitespace/garbage. Both live-captured payloads verified through the compiled node: `Retry after 9 seconds (Retry-After: 9)` and the ISO timestamp respectively.
- [X] T099 Extract `readHeader` to `nodes/Attio/core/readHeader.ts` with 10 unit tests. It was a private helper in `descriptions/shared.ts` with **no coverage**, and it matched only the exact and lower-cased header name (`bag[name] ?? bag[name.toLowerCase()]` called with an already-lowercase `'retry-after'`) — so a canonical `Retry-After` would have read as `undefined` and silently dropped the retry time from the message. Now a case-insensitive lookup, living in pure core so it is testable without pulling n8n types in (Principle VIII). In practice Attio serves HTTP/2, where header names are lowercase by spec, so the old code worked — this is hardening plus the missing coverage, not a live defect.
- [X] T089 Run `npx @n8n/scan-community-package @nodrel-dev/n8n-nodes-attio` — passes (SC-008)
